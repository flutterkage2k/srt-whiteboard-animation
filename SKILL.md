...
description: SRT 자막을 따뜻한 미색 종이 배경의 화이트보드 손그림 애니메이션으로 만든다. 자막 해석→그림 전략 출력→확인 후 통일 스타일 선화 생성→서사 의미 단위로 구역 주석→미리보기 도구 조정→MP4 렌더링. 연출은 구역 마스크 공개 방식(annotation.json / sequence / startMs / protectedRegions), 각 구역 내 그리기는 스트리밍 연속 펜 선(골격/그리드 ink→color). 사용자가 SRT 자막을 주며 "자막으로 화이트보드 애니메이션/손그림 영상 만들어줘", "SRT로 화이트보드 영상", "자막 순서대로 그려지는 영상" 등을 요청하면 트리거.
---

# SRT 白板动画（mask 编排 + stream 画法）

把 SRT 字幕转成白板手绘动画：**编排**沿用分区遮罩揭示（按叙事顺序逐区域揭示、未开始区域完全隐藏、重叠用 `protectedRegions` 保护）；**画法**换成流式笔迹——每个区域在自己的允许掩码内，笔尖沿骨架/网格连续滑行落墨（起笔 ink → 添彩 color），所有区域共享一张持久画布，已画完的区域保留在画布上。所有面向用户的说明、分镜、配置和界面文字必须使用韩语（한국어）。
...
4. **模块字段：** 每个元素含 `sequence`、`narrativeRole`、`subtitle`、`region`、`reveal`、`handPath`。`narrativeRole` 用韩语（한국어）说明其在字幕中的叙事作用；`subtitle` 存该区域对应的字幕文本（来自 SRT，供预览台联动与后续用途）；`sequence` 从 1 起连续。
